...
- 文件夹：拖拽合并创建（或右键 App「移入新文件夹」/ 空白处「新建文件夹」）；点击从图标位置缩放展开为居中玻璃卡片（iOS 同款两段动画），支持行内重命名、卡片内拖动排序、拖出卡片移出；内容超过 3×3 在卡片内垂直滚动
  <img src="docs/screenshots/folder.png" width="340" alt="文件夹卡片展开：玻璃卡片居中，行内重命名，内容超 3×3 卡片内滚动">
- 拖拽：**随时**拖动图标排序（不必先进编辑模式）——拖到占用格边缘出现「空位让位」的插入预览（iOS 式半格判定），拖到图标中心悬停片刻合并建文件夹；从 Finder 拖入 .app 即添加到桌面末尾
- 整理模式（iOS 抖动同款）：右键「整理桌面…」、长按 0.5s 或拖动图标均可进入；图标抖动并出现 ⊖ 移除徽章，点选图标多选后可批量删除；「＋ 添加」「✓ 完成」，Esc 或点面板空白处退出
  <img src="docs/screenshots/edit-mode.png" width="340" alt="整理模式：顶部整理条（多选删除 / 添加 / 完成），图标抖动 + ⊖ 移除徽章">
